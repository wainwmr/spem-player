...
const spemaudio = new Audio(defaultaudiofile);
...
const pSBC = (p, c0, c1, l) => {
  ...
  // eslint-disable-next-line eqeqeq, no-unused-expressions, no-nested-ternary, no-sequences
  ...
  // eslint-disable-next-line max-len, no-unused-expressions, no-sequences
...
pSBC.pSBCr = (d) => {
  ...
  if (n > 9) {
    ...
    // eslint-disable-next-line no-unused-expressions, eqeqeq, no-sequences
    ...
    // eslint-disable-next-line max-len, no-unused-expressions, no-bitwise, no-mixed-operators, no-sequences, eqeqeq
    if (n == 9 || n == 5)x.r = d >> 24 & 255, x.g = d >> 16 & 255, x.b = d >> 8 & 255, x.a = Math.round((d & 255) / 0.255) / 1000;
    // eslint-disable-next-line no-unused-expressions, no-bitwise, no-mixed-operators, no-sequences
...
function paintCanvas() {
  ...
  for (let c = 0; c < 8; c += 1) {
    const { parts } = choirs[c];
    for (let p = 0; p < 5; p += 1) {
      const startY = c * choirHeight + p * partHeight;

      const { ranges } = parts[p];
      for (let i = 0; i < ranges.length; i += 1) {
        const [from, to] = getRange(ranges[i]);
        ctx.beginPath();
...
// Not convinced the Maths is right here...
...
canvas.addEventListener("touchstart", (evt) => {
  ...
  canvas.addEventListener("touchmove", (em) => {
    pauseSpem();
    touchBar = getTouchPos(em);
    setBar(Number(touchBar));
    userInputChanged();
    evt.preventDefault();
  });
...
window.onload = calculateCanvasSize;
window.onresize = calculateCanvasSize;
