...
The script queries the current period's Netlify build-minute usage and GitHub Actions usage. Status thresholds — watch (≥ 50%), throttle (≥ 75%), and critical (≥ 90%) — apply to actual usage, while a linearly projected end-of-period usage can raise an early warning up to throttle (75%) when actual usage is below 90%. This prevents a single high-burn day early in the period from opening a false critical issue while still surfacing an unsustainable burn rate. On days with no PRs merged in the past 24 hours and both actual and projected usage below the watch threshold (50%), the Telegram message is skipped to reduce noise; watch and worse always send regardless of PR activity. When the message is sent, it takes the form `Netlify <n>% · GitHub <n>% — <status>` (the percentages are actual usage; the status reflects the worse of actual and projected, with projection capped at throttle unless actual usage is already critical), appending `· N PR(s) merged` when at least one PR merged in the past 24 hours. If either service actually reaches **90%** of its quota, the status becomes `STOP` (with a 🚨 prefix) and the workflow opens a critical GitHub issue containing a runbook for reducing build-minute consumption; projection alone cannot open a critical issue.

The `.github/monitor-series.json` file holds the daily `{ date, netlifyCurrent, githubMinutes, mergedPRs, summary, source }` series. `mergedPRs` is the number of PRs merged on that day; `summary` is a pre-computed snapshot `{ netlifyPct, netlifyProjected, netlifyEmoji, githubPct, githubProjected, githubEmoji, mergedPRs }` so agents can read status without recalculating. The `source` field is `"logged"` for entries written by the monitor and `"backfill"` for the one-time seed populated from API/GitHub run history and Netlify daily build minutes.
