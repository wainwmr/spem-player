...
Deploy previews are generated by the `netlify-preview.yml` GitHub Actions
workflow, which builds the site and deploys it to Netlify with a predictable
alias (`pr-NUMBER`). A bot comment with the preview URL is added to each PR.
