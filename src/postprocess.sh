...
for i in ../public/svg/modern/spem-choir*.svg; do
  echo -n "  [" >> $outfile
...
echo "export const scorebars_early = [" >> $outfile
for i in ../public/svg/early/spem-choir*.svg; do
  echo -n "  [0, " >> $outfile
  grep -B 2 '<tspan>[0-9]*</tspan>' "$i" | sed -n 's/^.*translate(\([^,]*\),.*$/\1/p' | sort -n | sed 's/$/, /' | tr -d '\n' >> $outfile
  echo "]," >> $outfile
done
...
for i in `find ../public/svg/ -type f -print | sort`; do
  sed -i '' -E '1,1s/ height="[0-9.]+[a-zA-Z]*"//g; 1,1s/ width="[0-9.]+[a-zA-Z]*"//g' "$i"
done

echo "`date`: Setting viewBox of early/spem-choir*.svg"
for i in ../public/svg/early/spem-choir*.svg; do
